...
public interface RolesRepository extends JpaRepository<Roles, Integer>,JpaSpecificationExecutor<Roles> {
	 ...
	 * @return
	 */
	@Query(value="SELECT role_id,roles_create_time,roles_explains,roles_last_update_time,roles_name FROM tb_roles WHERE role_id NOT IN (:ids)",nativeQuery=true)
	 ...
	 * @return
	 */
	@Query(value="SELECT * FROM tb_roles r LEFT JOIN tb_userrole u ON u.role_id=r.role_id WHERE u.user_id=:uid",nativeQuery=true)
	 ...
	 * @return
	 */
	@Query(value = "INSERT INTO tb_userrole (role_id,user_id) VALUES(:roleId,:userId)", nativeQuery = true)
	@Modifying
	@Transactional
	 ...
	 * @return
	 */
	@Query(value = "DELETE FROM tb_userrole WHERE role_id=:roleId AND user_id=:userId", nativeQuery = true)
	@Modifying
	@Transactional
