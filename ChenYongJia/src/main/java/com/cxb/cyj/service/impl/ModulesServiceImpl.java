...
import com.cxb.cyj.dao.ModulesRepository;
...
public class ModulesServiceImpl implements ModulesService {
	...
	private void setChildrens(List<Modules> parentList) {
		for (Modules m : parentList) {
			...
			System.out.println("*****************************************************设置子菜单=>" + m.getModulesName());
			// 如果没有子菜单则递归结束
	...
	public void setChildren(List<Modules> parentList, List<Integer> moduleId) {
		for (Modules m : parentList) {
			// 查询出子菜单
			List<Modules> childrenList = this.queryChildrenById(m.getModulesId(), moduleId);
			// 如果没有子菜单则递归结束
			if (childrenList != null && !childrenList.isEmpty()) {// 有子菜单
				// 设置子菜单
				m.setChildren(childrenList);
	...
	private void setRoleSetModuleTreeChildrens(List<Modules> parentList,
		...
		for (Modules m : parentList) {
			// 查询出子菜单
			List<Modules> childrenList = this.queryChildrenByIds(m.getModulesId());
			// 如果没有子菜单则递归结束
			if (childrenList != null && !childrenList.isEmpty()) {// 有子菜单
				// 设置子菜单
				m.setChildren(childrenList);
				...
				this.setRoleSetModuleTreeChildrens(childrenList, moduleId);
					...
					// 设置是否选中,把角色拥有的模块选中
				if (moduleId.contains(m.getModulesId()))
					m.setChecked(true);
			}
		}
	}
