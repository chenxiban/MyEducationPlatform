...
(function(r) {
	...
	(function(n) {
		...
		E.prototype.symbols.connector = function(c, b, a, k, d) {
			...
			d = d && d.anchorY;
			var f, m, h = a / 2;
			y(e) && y(d) && (f = ["M", e, d], m = b - d, 0 > m && (m = -k - m), m < a && (h = e < c + a / 2 ? m : a - m), d > b + k ? f.push("L", c + h, b + k) : d < b ? f.push("L", c + h, b) : e < c ? f.push("L",
				c, b + k / 2) : e > c + a && f.push("L", c + a, b + k / 2));
			return f || []
		};
					...
					for(m = c.length, d = 0; d < m; d += 1) {
						...
						if(0 < d && (e = Math.abs(b.chartX - a.chartX), f = Math.abs(b.chartY - a.chartY), e = Math.max(e, f), 16 < e))
							for(e = Math.ceil(e / 16), f = 1; f < e; f += 1) k.push({
								...
								chartCenterY: a.chartCenterY +
									f / e * (b.chartCenterY - a.chartCenterY),
		...
		z.prototype.checkClearPoint = function(c, b, a, k) {
			...
			for(t = 0; t < v.boxesToAvoid.length; t += 1)
				if(l = v.boxesToAvoid[t], g =
					c + a.width, p = b, n = b + a.height, !(c > l.right || g < l.left || p > l.bottom || n < l.top)) return !1;
			for(t = 0; t < v.series.length; t += 1)
				if(p = v.series[t], l = p.interpolatedPoints, p.visible && l) {
					for(g = 1; g < l.length; g += 1) {
						...
						}!h && !f || this === p && !q || (n = c + a.width / 2 - l[g].chartX, r = b + a.height / 2 -
							l[g].chartY, d = Math.min(d, n * n + r * r))
					}
					...
					if(a.visible && !a.isSeriesBoosting && g) {
						...
						if(m.length) {
							if(m.sort(function(a, b) {
								...
								anchorX: h.connectorPoint &&
									h.connectorPoint.plotX + l,
